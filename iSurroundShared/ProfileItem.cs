...
using iSurroundShared.Resources;
...
namespace iSurroundShared
{
    ...
    public class ProfileItem : IComparable<ProfileItem>, IEquatable<ProfileItem>
    {
        ...
        internal static string AppDataPath = System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "iSurround");
        ...
        public ProfileItem()
        {
            ...
            // loading the DisplayProfiles_2.0.json into iSurround next time.
        ...
        public bool IsValid()
        {
            // 只检查显示配置的有效性，不检查图标文件（因为新建时没有图标）
            try
            {
                var nv = NVIDIALibrary.GetLibrary();
                var amd = AMDLibrary.GetLibrary();
                var win = WinLibrary.GetLibrary();

                if (nv.IsInstalled && !nv.IsValidConfig(_nvidiaDisplayConfig))
                    return false;
                if (amd.IsInstalled && !amd.IsValidConfig(_amdDisplayConfig))
                    return false;
                if (!win.IsValidConfig(_windowsDisplayConfig))
                    return false;
                return true;
            }
            catch
